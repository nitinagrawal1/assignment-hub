...
const Contact = (props: { contactdataNumber: string }) => {
    ...
    // 2. Updated and fixed reset function
    const reset = () => {
        setFormData({
            name: "",
            email: "",
    ...
    const handleSubmit = async (e: any) => {
        ...
        fetch("https://formsubmit.co/ajax/assignsolutionscomp@gmail.com", {
            method: "POST",
            headers: { "Content-type": "application/json" },
            // 3. Updated handleSubmit body
            body: JSON.stringify({
                name: formData.name,
                    ...
                    <div className="flex flex-col gap-14 xl:gap-24">
                        <div className="flex flex-col xl:flex xl:flex-row items-start gap-8">
                            <div className="flex items-center py-3 gap-4 md:gap-8 w-full max-w-xl">
                                <span className="bg-primary dark:text-secondary py-1.5 px-2.5 text-base font-medium rounded-full">{contactdataNumber ? contactdataNumber : 9}</span>
