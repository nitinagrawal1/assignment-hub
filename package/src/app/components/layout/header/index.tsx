...
const Header = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                // FILTER MENU ITEMS HERE ⬇️
                const cleanedMenu = data.MenuData.filter((item: any) =>
                    item.name !== "Blog" &&
                    item.name !== "Docs"
                );
    ...
    return (
        <header className={`fixed top-0 z-50 w-full border-t-4 border-primary transition-all duration-500 ease-in-out
            before:content-[''] before:absolute before:top-0 before:left-0 before:w-full before:h-0 before:bg-primary
            before:transition-all before:duration-500 before:ease-in-out ${sticky ? "before:h-full" : "before:h-0"}`}>
